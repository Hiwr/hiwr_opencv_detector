...
bool HiwrOpencvDetectorNodelet::configure( ){
    ...
    NODELET_DEBUG("[Hiwr_opencv_detector Nodelet][configure] spined %f %d %d \n", gf_quality_level_ ,  gf_min_distance_ ,  gf_max_corners_);
    return true;
}

void HiwrOpencvDetectorNodelet::onInit(){
    NODELET_DEBUG("[Hiwr_opencv_detector Nodelet][onInit] beginning");
    ...
    if(!configure()) {
        NODELET_ERROR("[Hiwr_opencv_detector Nodelet][onInit] configure failed!");
    }

    if(!private_nh_.getParam("video_stream", video_stream_name_)){
        NODELET_ERROR("[Hiwr_opencv_detector Nodelet][onInit] Problem recovering the video stream");
        return;
    }
    ...
    NODELET_DEBUG("[Hiwr_opencv_detector Nodelet][onInit] call thread loop");


    loop_thread_ = std::thread(&HiwrOpencvDetectorNodelet::loop , this);

    NODELET_DEBUG("[Hiwr_opencv_detector Nodelet][onInit] end");
}

// processing the image here
void HiwrOpencvDetectorNodelet::callback(const sensor_msgs::ImageConstPtr& msg){
    NODELET_DEBUG("[Hiwr_opencv_detector nodelet] callback : beginning");
    ...
    try{
        ...
        // facetracking
        //if(im_processed){
        NODELET_DEBUG("[Hiwr_opencv_detector Nodelet] callback : im_ptr ok");

        if(!im_available_){
            // collecting the infos for frame
            frame_ = im_ptr_->image;

            NODELET_DEBUG("[Hiwr_opencv_detector Nodelet] callback : frame type : %d", frame_.type());

            // cvtColor(frame, frame, CV_8UC1);

            NODELET_DEBUG("[Hiwr_opencv_detector Nodelet] callback : frame type 2 : %d", frame_.type());
    ...
    NODELET_DEBUG("[Hiwr_opencv_detector nodelet] callback : end");
...
void HiwrOpencvDetectorNodelet::loop(){
    ...
    while(ros::ok()){
        if((im_ptr_!=NULL) && im_available_){
            NODELET_DEBUG("[Hiwr_opencv_detector Nodelet] loop : display");
            processTrackingFrame(frame_);
            #ifdef DEBUG_DISPLAY
                cv::imshow("Debug", frame_);
                cv::waitKey(20);
            #endif
            im_available_ = false;
        }else usleep(100);
...
void HiwrOpencvDetectorNodelet::processTrackingFrame(Mat frame ) {
    ...
    {
        NODELET_DEBUG("[Hiwr_opencv_detector Nodelet] Processing detection");
        ...
        return;
        NODELET_DEBUG("[Hiwr_opencv_detector Nodelet] End detection");
        ...
        if(keypoints_.size().height<5){
            NODELET_DEBUG("[Hiwr_opencv_detector Nodelet] tracking : less than 5 keypoints");    // -> OK
            detect_box_.width =0;
    ...
    NODELET_DEBUG("[Hiwr_opencv_detector Nodelet] procees tracking : before drawing");
